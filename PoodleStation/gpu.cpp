...
void GPU::draw_quad(bool textured, bool shaded)
{
    ...
    draw_tri(tri1, textured, shaded);
    draw_tri(tri2, textured, shaded);
}
...
void GPU::draw_tri(Vertex vertices[], bool textured, bool shaded)
{
...
void GPU::write_GP0(uint32_t value)
{
    ...
    {
        //printf("Transfer: %d\n", transfer_x);
        *(uint32_t*)&VRAM[(transfer_x + (transfer_y * 1024)) * 1] = value;
        transfer_x += 2;
        if (transfer_x >= transfer_x_bound)
        {
            transfer_y++;
