...
EmuWindow::EmuWindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    connect(&emuthread, SIGNAL(completed_frame(uint32_t*, int, int)),
            this, SLOT(draw_frame(uint32_t*, int, int)));
...
int EmuWindow::init(int argc, char** argv)
{
    ...
    {
        printf("Failed to load PSX BIOS from %s\n", bios_name);
        return 1;
    }
    printf("Loaded PSX BIOS.\n");
    uint8_t* BIOS = new uint8_t[1024 * 512];
    BIOS_file.read((char*)BIOS, 1024 * 512);
    BIOS_file.close();
...
void EmuWindow::draw_frame(uint32_t *buffer, int w, int h)
{
    if (!buffer)
        return;
